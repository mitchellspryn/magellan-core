...
void set_motor_current_limit(uint8_t motor_address, uint8_t write_command_id, uint8_t read_command_id, uint32_t max_amps, bool &verify_flag, std::vector<uint8_t> &received_data, ros::Publisher &publisher, const char* motor_name);
...
void drive(double left_throttle, double right_throttle)
{
         ...
         (!drive_motor(RIGHT_ROBOCLAW_ADDRESS, motor_br_command_id, motor_br_value, num_retries, right_response_valid, right_response, right_motor_controller_publisher)) )
    {
        if (ros::ok())
        {
            ROS_FATAL("Could not set motor controller values. Out of panic, attempting to stop both motors.");
...
void set_motor_current_limit(uint8_t motor_address, uint8_t write_command_id, uint8_t read_command_id, uint32_t max_amps, bool &verify_flag, std::vector<uint8_t> &received_data, ros::Publisher &publisher, const char* motor_name)
{
    ...
    // There is a scenario in which the system is shutting down, and the serial port node is dead.
    // In that case, then the stop command is being sent to the node already.
    if (ros::ok())
    {
        ROS_FATAL("Could not set motor current limiting for motor name %s. Out of safety, shutting down.", motor_name);
        ros::shutdown();
    }
}
...
bool initialize_motor_controllers()
{
    set_motor_current_limit(RIGHT_ROBOCLAW_ADDRESS, CMD_SET_M1_MAX_CURRENT, CMD_GET_M1_MAX_CURRENT, MOTOR_CURRENT_LIMIT_AMPS, right_response_valid, right_response, right_motor_controller_publisher, "right_front");
