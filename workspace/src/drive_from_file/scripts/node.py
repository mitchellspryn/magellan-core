...
from mangellan_messages.msg import MsgMagellanDrive

def parse_command_line_args():
    filtered_args = rospy.myargv(args=sys.argv)

    parser = argparse.ArgumentParser(description='Command line arguments for drive node')
    parser.add_argument(name='--drive-file', dest='drive_file', type=str, required=True, help='The file to read for driving')
...
def read_drive_file(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if len(split_line != 3):
                ROS_FATAL('Improper line found in drive file {0} at line {1}: "{2}". Expected three figures separated by space.'.format(filename, line_number, line))
                return None
            ...
            drive_commands.add( (left_wheel_speed, right_wheel_speed, sleep_time) )
            line_number += 1
...
def run_drive_commands(drive_commands, publisher):
    for command_index in range(0, len(drive_commands), 1):
        drive_command = drive_commands[i]

        rospy.loginfo('Sending command ({0}, {1}) for {2} seconds. Command {3}/{4}'.format(drive_command[0], drive_command[1], drive_command[2], command_index, len(drive_commands)))

        message = MsgMagellanDrive()
        message.left_throttle = drive_command[0]
        message.right_throttle = drive_command[1]
        message.header.stamp = rospy.get_time()
    ...
    final_message.header.stamp = rospy.get_time()
